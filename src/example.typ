...
/// if no language has been specified. Code in typst markup lanugage 
/// (`lang: typ`) is automatically evaluated in markup mode. 
/// 
/// - code (raw): Raw object holding the example code. 
/// - scope (dictionary): Additional definitions to make available for the 
///          evaluated example code.
/// - inherited-scope (dictionary): Definitions that are made available to the 
///          entire parsed module. This parameter is only used internally.
#let example(
  code, 
  dir: ltr,
  scope: (:),
